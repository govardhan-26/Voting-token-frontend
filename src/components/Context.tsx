...
function ElectionStateProvider({ children }) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const initialInstance = new Identity(
        PubKey(toHex(myPubkey)),
        toByteString(Data_On_chain, true)
      );
      ...
      console.log(
        `I issue ${vote_satoshisIssued} vote_token: ${vote_deployTx.id}`
      );
  ...
  const handleTransfer = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setmyAddress(addr1);
    console.log(`I send 1 to ${addr}: ${transferToAliceTx.id}`);
